...
def parse_opt():
    parser = argparse.ArgumentParser(description='InceptionV3 Training')
    ...
    parser.add_argument('--output_dir', type=str, default='./results/inceptionv3', 
...
if __name__ == "__main__":
    ...
    # 设置数据转换 - InceptionV3 需要 299x299 的输入尺寸
    if args.dataset == 'cifar10':
        # CIFAR10数据集转换 - 需要调整大小到299x299
    ...
    # 修改最后的全连接层以适应CIFAR10的10个类别
    model.fc = torch.nn.Linear(model.fc.in_features, 10)
    # 设置辅助分类器
    model.AuxLogits.fc = torch.nn.Linear(model.AuxLogits.fc.in_features, 10)
    
    model.to(device)
    print("创建模型: InceptionV3")
    ...
    for i in range(args.epochs):
        ...
        if is_best:
            torch.save(model.state_dict(), os.path.join(model_dir, "inceptionv3_best.pth"))
            print(f"保存最佳模型，准确率: {best_acc:.2f}%")

    # 保存最终模型
    torch.save(model.state_dict(), os.path.join(model_dir, "inceptionv3_final.pth"))
